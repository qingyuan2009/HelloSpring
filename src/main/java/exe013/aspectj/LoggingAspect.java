...
public class LoggingAspect {
    ...
    public void beforeMethod(JoinPoint joinPoint) {
        ...
        System.out.println("Logging前置通知=> 方法名" + methodName + " 参数 " + Arrays.toString(args));
    }
    ...
    public void afterMethod(JoinPoint joinPoint) {
        // 方法名
        String methodName = joinPoint.getSignature().getName();
        System.out.println("Logging后置通知=> 方法名" + methodName);
    }
    ...
    public void afterRuturningMethod(JoinPoint joinPoint, Object result) {
        // 方法名
        String methodName = joinPoint.getSignature().getName();
        System.out.println("Logging返回通知=> 方法名" + methodName + " 结果 " + result.toString());
    }
    ...
    // 如果改为NullPointException ex的话，当只有空指针异常才会触发通知
    public void afterThrowingMethod(JoinPoint joinPoint, Exception ex) { 
        // 方法名
        String methodName = joinPoint.getSignature().getName();
        System.out.println("Logging异常通知=> 方法名" + methodName + " 异常 " + ex);
    }
    ...
    public Object aroundMethod(ProceedingJoinPoint pjp) throws Throwable {
        ...
        try {
            // 调用目标方法
            // 前置通知
            System.out.println("Logging环绕前置通知=> 方法名" + methodName + " 参数 " + Arrays.toString(args));
            Object result = pjp.proceed();
            // 返回通知
            System.out.println("Logging环绕返回通知=> 方法名" + methodName + " 结果 " + result.toString());
            return result;
        }
        catch (Exception e) {
            // 异常通知
            System.out.println("Logging环绕异常通知=> 方法名" + methodName + " 异常 " + e);
        }
        finally {
            // 后置通知
            System.out.println("Logging环绕后置通知=> 方法名" + methodName);
        }
        return null;   
    }
